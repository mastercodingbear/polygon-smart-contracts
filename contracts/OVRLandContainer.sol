    ...
    AccessControlUpgradeable
{
    ...
    modifier isContainerOwner(uint256 _idContainer) {
        require(
            ownerOf(_idContainer) == _msgSender(),
    ...
    /**
     * @notice function to remove one land from a container
     */
    function removeLandFromContainer(uint256 _idContainer, uint256 _idLand)
        public
        isContainerOwner(_idContainer)
        containerFree(_idContainer)
    {
        require(
            landToContainer[_idLand] == _idContainer,
            "OVRLand not inside container"
        );
        require(
            _exists(_idContainer),
            "ERC721: query for nonexistent container"
        );
        // land is not inside the container anymore
        delete landToContainer[_idLand];
        //decrease number of lands in the container
        uint256 currentNumber = nLandsInContainer[_idContainer];
        nLandsInContainer[_idContainer] = currentNumber.sub(1);
        ...
        if (currentNumber - 1 > index) {
            for (uint256 i = index + 1; i < currentNumber; i++) {
                landIndex[containerToLands[_idContainer][i]] = landIndex[
                    containerToLands[_idContainer][i]
                ].sub(1);
                containerToLands[_idContainer][i - 1] = containerToLands[
                    _idContainer
                ][i];
            }
            delete containerToLands[_idContainer][currentNumber - 1];
        }
    ...
    /**
     * @notice function to add one land to a container
     */
    function addLandToContainer(uint256 _idContainer, uint256 _idLand)
        public
        isContainerOwner(_idContainer)
        landFree(_idLand)
    {
        ...
        uint256 currentNumber = nLandsInContainer[_idContainer];
        nLandsInContainer[_idContainer] = currentNumber.add(1);
        //add index of land
        landIndex[_idLand] = currentNumber;
        //insert land inside the land
        containerToLands[_idContainer][currentNumber] = _idLand;
     ...
     */
    function deleteContainer(uint256 _idContainer)
        public
        isContainerOwner(_idContainer)
        containerFree(_idContainer)
    {
        ...
        uint256 numberOfLands = nLandsInContainer[_idContainer];
        //the container doesn't exist anymore
        delete nLandsInContainer[_idContainer];
        for (uint256 i = 0; i < numberOfLands; i++) {
            delete landToContainer[containerToLands[_idContainer][i]];
            delete landIndex[containerToLands[_idContainer][i]];
            ...
            delete containerToLands[_idContainer][i];
        }

        _burn(_idContainer);
        emit ContainerDeleted(_idContainer, _msgSender(), block.timestamp);
    }
